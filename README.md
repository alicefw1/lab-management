...
> 系统界面采用**深空蓝黑 + 霓虹青紫**的科技风设计，含玻璃态卡片、网格背景、发光边框与实时数据图表，界面整洁、风格统一，符合软件著作权申请与项目答辩的规范要求。
...
- **主要功能**：实验室信息管理、实验室预约、设备预约、设备借用与归还、设备报修、设备采购、实验室耗材管理、危化品标准库、废弃物审核验收与处置、诚信评分、用户权限管理、综合数据统计分析
- **软件特色**：JWT 无状态鉴权、注解式权限控制、前后端分离、科技可视化大屏、诚信评分体系

> 申请软著时，需提交「源程序」与「软件说明书」。本项目的后端源码（`02_后端工程`）、前端源码（`03_前端工程完整代码`）与数据库脚本（`01_数据库脚本`）即为完整的源程序；`软件说明书.md` 与 `用户使用文档.md` 可作为申报材料。
